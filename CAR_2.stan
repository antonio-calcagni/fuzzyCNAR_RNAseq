...
model{
  ...
  for (i in 1:n){
    real mustar = (mu[i]*1.0)/K;
    real hlambda = h[i]*lambda;
    target += gamma_lpdf(h[i] | alpha_h, beta_h) + beta_lpdf(cstar[i] | 1e-6+mustar*hlambda,1e-6+hlambda-hlambda*mustar); 
  }
}
...
generated quantities{
  ...
  for (i in 1:n){
    ...
    c_rep[i] = rescale_inv(beta_rng(1e-6+(1.0*mu[i]/K)*(lambda*h_rep[i]),1e-6+(lambda*h_rep[i])-(lambda*h_rep[i])*(1.0*mu[i]/K)),lb,ub);
  }
